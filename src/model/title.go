...
func FindTitles(title Title) *Titles {
	var titles Titles
	if rows, err := db.Raw("SELECT d1.id, d1.title FROM doc d1 WHERE d1.deleted_at IS NULL AND d1.lang = ? AND d1.title LIKE ? AND (d1.alias_id IS NULL OR d1.alias_id = '00000000-0000-0000-0000-000000000000')", title.Lang, "%"+title.Title+"%").Rows(); err != nil {
		log.Print(err)
		return nil
	} else {
		for rows.Next() {
